...
if( !function_exists( 'ot_get_google_font' ) ) :
	...
	function ot_get_google_font($key = false, $interval = 604800 ){
		...
		if( ! empty($api_key) ){ // check if the api key is set
			...
			if ( !$last || ( ($now - $last ) > $interval ) || !$theme || $current_fonts == "" || !$current_fonts ) {
				...
				if ( 200 == wp_remote_retrieve_response_code( $google_response ) ) {
				...
				} else if (403 == wp_remote_retrieve_response_code( $google_response )) {

					function notice_api_limit_exceeded() {

						// Display message
						$html =  '<div class="error"><p>';
						$html .= "Daily limit for unauthenticated use of <strong>Google Fonts API</strong> exceeded. Please try it again later.";
						$html .= '</p></div>';
					...
					add_action( 'admin_notices', 'notice_api_limit_exceeded', 99);
			...
			if ( ! function_exists('notice_no_api_key') ) {
				function notice_no_api_key() {
					// Display message
					$html =  '<div class="error"><p>';
					$html .= "You need to set the Google Fonts API Key in order to use the Custom fonts features.";
					$html .= '</p></div>';
			...
			add_action( 'admin_notices', 'notice_no_api_key', 99);
		}
...
function ot_action_get_google_font_link(){
	...
	if (!is_admin()) {
		...
		$_ot_options = get_option( $ot_settings_id );
		...
		foreach( $_font_array as $index => $font ){
			...
			if( !empty($font) && is_array($font) ) :
				...
				if (isset($_db_font_cache) && is_array($_db_font_cache)) {
